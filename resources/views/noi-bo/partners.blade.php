...
@section('title', 'Quản lý Đối tác - Quản trị ITC')
...
<style>
    ...
    .btn-create-partner:active {
        transform: translateY(0);
    }
    .btn-create-partner svg {
        width: 16px;
        height: 16px;
    }
    .alert {
...
<div class="dashboard">
    <div class="partners-container">
        ...
        <!-- Đối Tác Header và Nút Tạo mới -->
        <div class="partner-card">
            <div class="partner-card-header">
                <div>
                    <h2>Quản lý Đối tác - Khách hàng</h2>
                    <p class="subtitle font-sans">Hiển thị, tìm kiếm và thay đổi logo đối tác khách hàng liên kết.</p>
                </div>
                <a href="/admin/partners/create" class="btn-create-partner">
            ...
            <div class="filter-bar">
                <div class="search-input-wrapper">
                    ...
                    <input type="text" id="partnerSearchInput" class="search-control" placeholder="Tìm kiếm đối tác...">
                </div>
            ...
            <div class="partner-table-wrapper">
                <table class="partner-table">
                    ...
                    <tbody id="partnerTableBody">
                        @forelse($partnersList as $item)
                            <tr class="partner-row" 
                                data-name="{{ strtolower($item->name) }}" 
                                data-group="{{ $item->group }}"
                                data-status="{{ $item->is_visible ? 'visible' : 'hidden' }}">
                                <td>
                                    <img src="{{ $item->logo_path }}" alt="{{ $item->name }}" class="partner-logo-preview">
                                </td>
                                <td>
                                    <span class="partner-name-cell">{{ $item->name }}</span>
                                </td>
                                <td>
                                    <span class="partner-group-cell font-sans">
                                        @if($item->group == 'gov')
                                            Cơ quan Bộ Ngành
                                ...
                                <td>
                                    <div class="actions-cell" style="justify-content: center;">
                                        <!-- Edit Action -->
                                        <a href="/admin/partners/{{ $item->id }}/edit" class="btn-action" title="Chỉnh sửa">
                                            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </a>
                                        <!-- Delete Action -->
                                        <form method="POST" action="/admin/partners/delete" onsubmit="return confirm('Bạn có chắc chắn muốn xóa đối tác này?');" style="margin: 0;">
                                            @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    // 1. Instant Search & Group Filtering
        ...
        if (rows.length > 0) {
            if (visibleCount === 0) {
                if (!emptyRow) {
                    ...
                    document.getElementById('partnerTableBody').appendChild(tr);
                } else {
    ...
    // 2. AJAX Status Toggle for partners
    const checkboxes = document.querySelectorAll('.status-toggle-checkbox');
    checkboxes.forEach(chk => {
        chk.addEventListener('change', function() {
            const partnerId = this.getAttribute('data-id');
            const statusText = document.getElementById(`status-text-${partnerId}`);
            
            statusText.innerText = '...';
            statusText.className = 'status-text';

            fetch('/admin/partners/toggle-status', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const row = chk.closest('.partner-row');
                    if (row) {
